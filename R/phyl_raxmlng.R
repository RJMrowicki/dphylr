...
phyl_raxmlng <- function (
  raxmlng_dir, aln_file, part_file, rseed = NULL, bscr_dir, run = FALSE
) {
  ...
  readr::write_lines(  # write the following lines...
    c(
      ...
      #   '--bs-trees 5000 --bs-cutoff 0.03'),  # bootstrapping (autoMRE{n} / 5000?)
      # 
      ...
      paste0(  # perform standard bootstrapping:
        ...
        paste0('--threads 4 --force --seed ',rseed,' '),  # no. threads (forced), random seed
        '--bootstrap --bs-trees 5000'),  # bootstrapping (autoMRE{n} / 5000?)
